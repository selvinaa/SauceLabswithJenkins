...
public class Driver {
    ...
    public static void initialize(String browser){
        if (driver != null )
            return;
        if(ConfigReader.readProperty("runInSauceLab").equalsIgnoreCase("true")){
            driver = getRemoteDriver();// for SAUCE LAB WEBSITE
            return;
        }
        switch (browser) {
            case "chrome":
    ...
    public static WebDriver getRemoteDriver(){
        ...
        try {
            DesiredCapabilities capabilities = DesiredCapabilities.chrome();// initialized crome driveer
